...
class CreateNotificationsMenu(SubWindow):
    ...
    def __init__(self, parent: ElementType, icon: QIcon, \
        ...
        uic.loadUi(fr"{cwd}logic\createNotifications\create_notifications_menu.ui", self)
        ...
        self.actual_time = str(self.database.get_current_notifications_time()[0])
        self.actual_title = str(self.database.get_current_notifications_title()[0])
        self.actual_message = str(self.database.get_current_notifications_message()[0])
        self.actual_launch = str(self.database.get_current_notifications_title()[0])
    ...
    def delete_from_db(self):
        ...
        """
        time, title, message, launch = get_text(self, ("time_line", "title_line", "message_line", "url_line"))
        current_working_directory = cwd
        self.database.delete_notification(time, title, message, launch)
        update_window(self)
        self.database = l_database.BrainDatabase(fr"{current_working_directory}\src\brain_mine.db")
